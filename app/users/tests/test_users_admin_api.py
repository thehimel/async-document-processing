...
class TestUsersAdminAPI:
    ...
    async def test_get_user_returns_200(
        ...
        response = await client_admin.get(routes.users_by_id(test_user.id))
    ...
    async def test_user_not_found_returns_404(self, client_admin, routes, method):
        ...
        if method == "get":
            ...
            response = await client_admin.patch(url, json={"role": "user"})
        else:
    ...
    async def test_patch_user_returns_200(
        ...
        response = await client_admin.patch(routes.users_by_id(test_user.id), json=payload)
    ...
    async def test_delete_user_returns_204(
        ...
        response = await client_admin.delete(routes.users_by_id(test_user.id))
